 ...
 */
function apigee_devconnect_responsive_preprocess_html(&$variables) {
  ...
  drupal_add_css(".navbar .nav .open > .dropdown-toggle, .navbar .nav .active > .dropdown-toggle, .navbar .nav .open.active > .dropdown-toggle {color: $header_text_color}", array('group' => CSS_THEME, 'type' => 'inline'));
  ...
  drupal_add_css(".btn, .btn:hover {color: $button_text_color; background: $button_background_color}", array('group' => CSS_THEME, 'type' => 'inline'));
  drupal_add_css(".footer .navbar .nav > li > a, .footer .navbar .nav li a:hover{background: transparent}", array('group' => CSS_THEME, 'type' => 'inline'));
  drupal_add_css(".footer .navbar .nav > li > a.active, .footer .navbar .nav .active > a {background: transparent}", array('group' => CSS_THEME, 'type' => 'inline'));
}

/**
 * Preprocessor for theme('page').
 */
function apigee_devconnect_responsive_preprocess_page(&$variables) {
  ...
  if($variables['main_menu']) {
    ...
    $variables['main_menu'] = apigee_base_responsive_menu_navigation_links($tree);

    // Build list
    $variables['primary_nav'] = theme('apigee_base_responsive_links', array(
  ...
  if (module_exists('menu')) {
    ...
    if ($variables['secondary_menu']) {
      ...
      $variables['secondary_menu'] = apigee_base_responsive_menu_navigation_links($tree);

      // Build list
      $variables['secondary_nav'] = theme('apigee_base_responsive_btn_dropdown', array(
 ...
 */
function apigee_devconnect_responsive_preprocess_node(&$variables) {
 ...
 */
function apigee_devconnect_responsive_preprocess_region(&$variables, $hook) {
 ...
 */
function apigee_devconnect_responsive_form_comment_form_alter(&$form, &$form_state) {
  hide($form['subject']);
  hide($form['author']);
 ...
 * hook_css_alter
 */
function apigee_devconnect_responsive_css_alter(&$css) {
  if (isset($css['misc/ui/jquery.ui.theme.css'])) {
    $css['misc/ui/jquery.ui.theme.css']['data'] = drupal_get_path('theme', 'apigee_devconnect_responsive') . '/jquery_ui/jquery-ui-1.9.0.custom.css';
  }

  // Add/Remove apigee_devconnect_responsive_wide_layout.css depending on theme setting value (defaults to enabled)
  if (theme_get_setting('wide_layout') == 0) {
    unset($css[drupal_get_path('theme', 'apigee_devconnect_responsive') . '/css/apigee_devconnect_responsive_wide_layout.css']);
  }
}
