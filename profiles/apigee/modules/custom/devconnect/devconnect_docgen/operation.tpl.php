...
extract($variables);
$type = $variables['type'];
$path = 'field_' . $type . '_model_path';
$path = $node->$path;
$path = $path['und'][0]['value'];
$verb = 'field_' . $type . '_model_verb';
$verb = $node->$verb;
$verb = taxonomy_term_load($verb['und'][0]['tid']);
$verb = $verb->name;

// Multiples need to be rendered
$auth = render($content['field_' . $type . '_model_authschemes']);
$types = render($content['field_' . $type . '_model_types']);
$tags = render($content['field_' . $type . '_model_tags']);
